...
INSERT INTO `components` (`id`, `name`, `version`, `size`, `description`, `dynamics_direction`, `module_type`, `resource_type`) VALUES (1, '冲击前处理程序', '1.0', '2.5MB', '这是冲击前处理程序', '冲击', '前处理', 'CPU');
INSERT INTO `components` (`id`, `name`, `version`, `size`, `description`, `dynamics_direction`, `module_type`, `resource_type`) VALUES (2, 'CPU冲击求解器程序', '1.0', '2.6MB', '这是CPU冲击求解器', '冲击', '求解器', 'CPU');
INSERT INTO `components` (`id`, `name`, `version`, `size`, `description`, `dynamics_direction`, `module_type`, `resource_type`) VALUES (3, 'GPU冲击求解器程序', '1.0', '2.7MB', '这是GPU冲击求解器', '冲击', '求解器', 'GPU');
INSERT INTO `components` (`id`, `name`, `version`, `size`, `description`, `dynamics_direction`, `module_type`, `resource_type`) VALUES (4, '结构前处理程序', '1.0', '2.8MB', '这是结构前处理程序', '结构', '前处理', 'CPU');
INSERT INTO `components` (`id`, `name`, `version`, `size`, `description`, `dynamics_direction`, `module_type`, `resource_type`) VALUES (5, '结构求解器程序', '1.0', '2.9MB', '这是结构求解器', '结构', '求解器', 'CPU');
INSERT INTO `components` (`id`, `name`, `version`, `size`, `description`, `dynamics_direction`, `module_type`, `resource_type`) VALUES (6, '多体前处理程序', '1.0', '3.0MB', '这是多体前处理程序', '多体', '前处理', 'CPU');
INSERT INTO `components` (`id`, `name`, `version`, `size`, `description`, `dynamics_direction`, `module_type`, `resource_type`) VALUES (7, '多体求解器程序', '1.0', '3.1MB', '这是多体求解器', '多体', '求解器', 'CPU');
INSERT INTO `components` (`id`, `name`, `version`, `size`, `description`, `dynamics_direction`, `module_type`, `resource_type`) VALUES (8, '通用后处理程序', '1.0', '3.2MB', '这是通用后处理程序', '通用', '后处理', 'CPU');
...
CREATE TABLE `file_meta` (
  ...
  `preview_image_size` bigint DEFAULT NULL COMMENT '预览图大小(字节)',
...
INSERT INTO `license_apply` VALUES ('20303441803563458560020','04:A2:06:DD:14:23','APPROVED','D:\\Project\\Java\\license\\uploads\\license_2030344990561660928.lic','solver-struct','solver','2026-03-24 00:00:00','2026-05-29 23:59:59',5,'2026-03-08 02:05:49','张三','Ming','LIC-2030344990645547008',20),('20339240303756165120020','11:22:33:44:55:66','VALID','D:\\Project\\Java\\license\\uploads\\license_2033924108876210176.lic','pre-impact','pre','2026-03-19 00:00:00','2026-04-30 23:59:59',3,'2026-03-17 23:10:52','张三','李四','LIC-2033924108922347520',20);
...
CREATE TABLE `mod_tasks` (
  ...
  `execution_mode` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT '执行模式：remote=后端服务器执行，local=用户端本地执行，空值=未选择执行模式',
  `compute_resource` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT '计算类型（GPU等）',
  `server_id` int DEFAULT NULL COMMENT '任务执行所在服务器ID',
  `server_name` varchar(255) COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT '任务执行所在服务器名称',
  `priority` int NOT NULL DEFAULT '2' COMMENT '高=1，中=2，低=3',
  `cpu_core_need` int NOT NULL DEFAULT '1' COMMENT 'CPU核心需求',
  `memory_need` int NOT NULL DEFAULT '4' COMMENT '内存需求(GB)',
  ...
  KEY `idx_mod_tasks_project_ctime` (`project_id`,`creation_time`) USING BTREE,
  KEY `creator` (`creator`) USING BTREE,
  KEY `idx_mod_tasks_remote_schedule` (`status`,`execution_mode`,`priority`,`creation_time`),
  KEY `idx_mod_tasks_server_id` (`server_id`),
...
INSERT INTO `mod_tasks` VALUES (3,'test1',20,'2026-03-16 10:57:09',1,'前处理','冲击','completed',NULL,1,'GPU-Server-01',2,1,4,100,'2026-03-17 22:38:03','2026-03-17 23:03:18',NULL),(4,'test2',20,'2026-03-17 23:09:16',1,'求解器','冲击','completed','GPU',1,'GPU-Server-01',2,1,4,100,'2026-03-17 23:09:37','2026-03-17 23:10:10',NULL);
...
CREATE TABLE `server` (
  ...
  `cpu_usage` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '(%)',
  `memory_usage` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '(%)',
  PRIMARY KEY (`id`)
...
INSERT INTO `server` VALUES (1,'1407','GPU-Server-01','192.168.1.101','GPU服务器','running',32,128,'15','17');
...
-- Dump completed on 2026-03-18 19:07:19
